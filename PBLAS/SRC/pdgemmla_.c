...
*
*  panel   (global input) STRUCTURE
*          The panel descriptor structure holds the metadata of the
*          panel buffer broadcasted for lookahead in LU factorization.
*          and panel information for the lookahead.
*
*  PMEM   (global and local input) Double array
*          The buffer holds global matrix data needed for trsm and gemm
*          operations for the lookahead.
*
*  ---------------------------------------------------------------------
*/
/*
...
*/
   Cblacs_gridinfo( ( ctxt = Ad[CTXT_] ), &nprow, &npcol, &myrow,
                    &mycol );
  // printf("Grid: %d %d, (Ai, Aj) = (%d, %d), (Ai, Aj) = (%d, %d), (Ai, Aj) = (%d, %d)\n", myrow, mycol, Ai, Aj, Bi, Bj, Ci, Cj);
...
*/
   ...
   if( ChooseAB )
   {
      PB_CpgemmlaAB( type, &DirAB, &DirBC, ( nota ? NOTRAN : TRAN ), ( notb ?
                     NOTRAN : TRAN ), *M, *N, *K, ((char *)ALPHA), ((char *)A),
                     Ai, Aj, Ad, ((char *)B), Bi, Bj, Bd, ((char *)BETA),
                     ((char *)C), Ci, Cj, Cd, panel, ((char *)PMEM) );
   }
   else if( ChooseBC )
   {
      PB_CpgemmlaBC( type, &DirAB, &DirBC, ( nota ? NOTRAN : TRAN ), ( notb ?
                     NOTRAN : TRAN ), *M, *N, *K, ((char *)ALPHA), ((char *)A),
                     Ai, Aj, Ad, ((char *)B), Bi, Bj, Bd, ((char *)BETA),
                     ((char *)C), Ci, Cj, Cd, panel, ((char *)PMEM) );
   }
   ...
   {
      PB_CpgemmlaAC( type, &DirAB, &DirBC, ( nota ? NOTRAN : TRAN ), ( notb ?
                     NOTRAN : TRAN ), *M, *N, *K, ((char *)ALPHA), ((char *)A),
                     Ai, Aj, Ad, ((char *)B), Bi, Bj, Bd, ((char *)BETA),
                     ((char *)C), Ci, Cj, Cd, panel, ((char *)PMEM) );
   }
