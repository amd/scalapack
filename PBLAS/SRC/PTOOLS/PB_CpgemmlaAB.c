/* ---------------------------------------------------------------------
*
*  -- PBLAS auxiliary routine (version 2.1.0) --
...
#ifdef __STDC__
void PB_CpgemmlaAB( PBTYP_T * TYPE, char * DIRECA, char * DIRECB,
...
*/
       ...
       !( PB_Cspan( K, BiR, BinbR, BnbR, BsrcR, BnprocsR ) ) )
   {
      ...
      {
        Xld = DESCA[LLD_];
        WA = Mptr( ((char *)A), Xii, Xjj, Xld, TYPE->size );
      }
      else
      {
        Xld = panel->lda;
...
*/
         TYPE->Fgemm( C2F_CHAR( NOTRAN ), C2F_CHAR( NOTRAN ), &Cmp, &Cnq, &K,
                      ALPHA, WA, &Xld, WB, &WBd0[LLD_], BETA, Mptr( C,
